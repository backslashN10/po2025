# TODO

- [x] uzupelnic diagramy o nowe funkcjonalnosci
- [x] ~~dodac ANSI i polepszyc graficzny CLI~~ dodać GUI
- [will do] dodac watki jako symulacja jakiegos polaczenia (moj pomysl to cykliczne powiadomienia z bazy danych czy wszystko dziala itd, drugi pomysl to wspolbiezne zapytania do bazy danych)
- [x] autoryzacja 2fa przez google token
- [ ] rozwinac te raporty, miesieczne itd
- [ ] symulacja awarii urzadzen po jakims czasie czy cos
- [x] logi (co, kto, kiedy, ~~z jakiego ip~~ )
- [ ] historia zmian
- [x] uzycie findby status i findby type dla devices
- [x] uzycie find by id, find by role dla userow
- [?] biedny backup
- [x] ogarniecie unique constraint jak sie odpala
- [x] fajnie jakby pokazywalo kto jest zalogowany
- [ ] dodać skrypt pozwalający uzupełnić baze przykładowymi danymi
